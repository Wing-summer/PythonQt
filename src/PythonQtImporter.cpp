...
// modified to do all the loading of Python files via an external file
// interface. In contrast to zipimport.c, this module also writes *.pyc files
...
PythonQtImport::getModuleInfo(PythonQtImporter *self, const QString &fullname) {
  ModuleInfo info;
  QString subname;
...
*/
int PythonQtImporter_init(PythonQtImporter *self, PyObject *args,
                          PyObject * /*kwds*/) {
  self->_path = NULL;
...
PyObject *PythonQtImporter_find_module(PyObject *obj, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "s|O:PythonQtImporter.find_module", &fullname,
                        &path))
    return NULL;

  // qDebug() << "looking for " << fullname << " at " << *self->_path;

  PythonQtImport::ModuleInfo info =
...
PyObject *PythonQtImporter_iter_modules(PyObject *obj, PyObject *args) {
  ...
  PythonQtObjectPtr importer =
      pkgutil.call("ImpImporter", QVariantList() << *self->_path);
  PythonQtObjectPtr result = importer.call(
      "iter_modules", QVariantList() << QString::fromUtf8(prefix));
  PyObject *o = result;
  Py_XINCREF(o);
  return o;
}
...
PyObject *PythonQtImporter_load_module(PyObject *obj, PyObject *args) {
  ...
  if (info.type == PythonQtImport::MI_PACKAGE ||
      info.type == PythonQtImport::MI_MODULE) {
    QString fullPath;
    QString fullCachePath;
    code =
        PythonQtImport::getModuleCode(self, fullname, fullPath, fullCachePath);
    if (code == NULL) {
      return NULL;
    }
    ...
    if (info.type == PythonQtImport::MI_PACKAGE) {
      ...
      fullpath =
          PyString_FromFormat("%s%c%s", self->_path->toUtf8().constData(), SEP,
      ...
      // set __package__ only for Python 3, because in Python 2 it causes the
      // exception "__package__ set to non-string"
#ifdef PY3K
      // The package attribute is needed to resolve the package name if it is
      // referenced as '.'. For example, when importing the encodings package,
      // there is an import statement 'from . import aliases'. This import would
      // fail when reloading the encodings package with importlib.
      err = PyDict_SetItemString(dict, "__package__",
                                 PyUnicode_FromString(fullname));
      if (err != 0) {
        Py_DECREF(code);
        Py_DECREF(mod);
        return NULL;
...
#ifdef PY3K
    mod = PyImport_ExecCodeModuleWithPathnames(
        fullname, code, fullPath.toUtf8().data(),
        fullCachePath.isEmpty() ? NULL : fullCachePath.toUtf8().data());
#else
    ...
    pathList.setNewRef(
        PythonQtConv::QStringListToPyList(QStringList() << *self->_path));
    ...
    if (result.isValid()) {
      // This will return a tuple with (file, pathname,
      // description=(suffix,mode,type))
      ...
      if (list.count() == 3) {
        ...
        // find_module opened a file to the release library, but that file
        // handle is ignored on Linux and Windows, maybe on MacOS also.
        list[2] = info.fullPath;
#endif
#endif
        // And call "load_module" with (fullname, file, pathname,
        // description=(suffix,mode,type))
        ...
        // Finally, we need to close the file again, which find_module opened
        // for us
...
PyTypeObject PythonQtImporter_Type = {
    PyVarObject_HEAD_INIT(DEFERRED_ADDRESS(&PyType_Type),
                          0) "PythonQtImport.PythonQtImporter",
...
void PythonQtImport::writeCompiledModule(PyCodeObject *co,
                                         const QString &filename, long mtime,
                                         long sourceSize) {
  FILE *fp;
...
PyObject *PythonQtImport::unmarshalCode(const QString &path,
                                        const QByteArray &data, time_t mtime) {
  PyObject *code;
  ...
  if (!PyCode_Check(code)) {
    Py_DECREF(code);
    PyErr_Format(PyExc_TypeError, "compiled module %.200s is not a code object",
                 path.toUtf8().constData());
    return NULL;
  }
  return code;
...
PyObject *PythonQtImport::compileSource(const QString &path,
  ...
  code =
      Py_CompileString(data.data(), path.toUtf8().constData(), Py_file_input);
  return code;
}

QString PythonQtImport::getCacheFilename(const QString &sourceFilename,
                                         bool isOptimizedFilename) {
#ifdef PY3K
  QFileInfo fi(sourceFilename);
  static QString cacheTag(PyImport_GetMagicTag());
  QString pycFilename = fi.absolutePath() + "/__pycache__/" + fi.baseName() +
                        "." + cacheTag + ".py";
#else
...
PyObject *PythonQtImport::getCodeFromData(const QString &path, int isbytecode,
                                          int /*ispackage*/, time_t mtime) {
  PyObject *code;
  ...
  if (!isbytecode) {
    ...
    if (!ok) {
      //    mlabErrorConst("PythonQtImporter","File could not be verified" <<
      //    path);
      return NULL;
    }
    if (qdata == " ") {
  ...
  if (isbytecode) {
    ...
    if (code) {
      ...
      QString cacheFilename =
          getCacheFilename(path, /*isOptimizedFilename=*/false);
      writeCompiledModule((PyCodeObject *)code, cacheFilename, time.toTime_t(),
                          /*sourceSize=*/qdata.length());
    }
  }
  return code;
}
...
/* Get the code object associated with the module specified by
   'fullname'. */
PyObject *PythonQtImport::getModuleCode(PythonQtImporter *self,
  ...
  for (zso = mlab_searchorder; *zso->suffix; zso++) {
    ...
    if (PythonQt::importInterface()->exists(test)) {
      ...
      // and unmarshalCode() in getCodeFromData() will always read an existing
      // *.pyc file, even if a newer *.py file exists. This is a release
      // optimization where typically only *.pyc files are delivered without
      // *.py files and reading file modification time is slow.
      if (isbytecode &&
          !PythonQt::importInterface()->ignoreUpdatedPythonSourceFiles()) {
...
QString PythonQtImport::replaceExtension(const QString &str,
                                         const QString &ext) {
  QString r;
...
PyObject *PythonQtImport::getCodeFromPyc(const QString &file) {
  ...
  QString pyc;
#ifdef PY3K
  // if the cache file in __pycache__ does not exist, look for cache file next
  // to source file
  ...
  if (PythonQt::importInterface()->exists(pyc)) {
    ...
    // and unmarshalCode() in getCodeFromData() will always read an existing
    // *.pyc file, even if a newer *.py file exists. This is a release
    // optimization where typically only *.pyc files are delivered without *.py
    // files and reading file modification time is slow.
...
PyDoc_STRVAR(mlabimport_doc, "Imports python files into PythonQt, completely "
                             "replaces internal python import");
...
void PythonQtImport::init() {
...
#else
  mod = Py_InitModule4("PythonQtImport", NULL, mlabimport_doc, NULL,
                       PYTHON_API_VERSION);
#endif

  PythonQtImportError = PyErr_NewException(
      const_cast<char *>("PythonQtImport.PythonQtImportError"),
  ...
  PyObject *classobj =
      PyDict_GetItemString(PyModule_GetDict(mod), "PythonQtImporter");
...
#ifndef WIN32
  // reload the encodings module, because it might fail to custom import
  ...
  if (encodingsModule != NULL) {
    ...
    // import it now, so that the search function is registered (a previous
    // import from the codecs module may have failed and it does not retry to
    // import it)
